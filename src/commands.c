 ...
 */
int change_dir_cmd(char **cmd)
{
	char *new_dir;

	if (cmd[1] == NULL ||
	    strcmp(cmd[1], "~") == 0 || strcmp(cmd[1], "~/") == 0) {
		const char *HOME;
		...
		if (getcwd(cwd, MAX_DIR_LEN) == NULL) {
			perror(ANSI_COLOR_RED "getcwd() error : "
			       ANSI_COLOR_RESET);
			return -1;
		}
...
int shell_cmd(char **cmd)
{
	...
	if (!strcmp(command, EXIT_CMD)) {
		printf(ANSI_COLOR_YELLOW "Exitting...\n\n" ANSI_COLOR_RESET);
		exit(0);
	} else if (!strcmp(command, CD_CMD)) {
		int ret_val = change_dir_cmd(cmd);
		return ret_val;
	}
